...
class Tensorboard:
    def __init__(self, config):
        ...
        config.run_name = "traCoCo({}-label, unsup_weight={})[{}]".format(
            str(config.labeled_num), str(config.unsup_weight), str(config.dataset)
        )
    ...
    def upload_wandb_table(self, info_dict):
        table = wandb.Table(
            data=info_dict["entire_prob_boundary"], columns=["boundary", "rate"]
        )
        wandb.log(
            {
                "pass_in_each_boundary": wandb.plot.bar(
                    table, "boundary", "rate", title="PASS_RATE Bar Chart"
                )
            }
        )
        table = wandb.Table(
            data=info_dict["max_prob_boundary"], columns=["boundary", "rate"]
        )

        wandb.log(
            {
                "max_prob_in_each_boundary": wandb.plot.bar(
                    table, "boundary", "rate", title="MAX_Prob Bar Chart"
                )
            }
        )
    ...
    def produce_2d_slice(self, image, label, pred):
        ...
        wandb.log({"volume_slice": [wandb.Image(grid_image, caption="x")]})

        outputs_soft = torch.softmax(pred, 1)
        image = (
            outputs_soft[0, 1:2, :, :, 20:61:10].permute(3, 0, 1, 2).repeat(1, 3, 1, 1)
        )
        grid_image = torchvision.utils.make_grid(image, 5, normalize=False)
        wandb.log({"output_slice": [wandb.Image(grid_image, caption="y_tilde")]})
        ...
        grid_image = torchvision.utils.make_grid(image, 5, normalize=False)
        wandb.log({"label_slice": [wandb.Image(grid_image, caption="y")]})
        return

    def produce_3d_gif(self, current_epoch, result, name="?", color="red"):
        current_path = os.path.join(self.visual_results_root, str(current_epoch))
        self._safe_mkdir(
            parent_path=self.visual_results_root, build_path=str(current_epoch)
        )
        ...
        for ii in [0, 90, 180, 270]:
            ...
            plt.savefig(
                os.path.join(current_path, name, "{}.png".format(ii)),
                bbox_inches="tight",
                pad_inches=0,
                dpi=200,
            )
        ...
        for filename in sorted(
            sorted(
                os.listdir(os.path.join(current_path, name)),
        ...
        wandb.log(
            {
                "{}_gif".format(name): wandb.Video(
                    current_path + "/{}.gif".format(name), fps=4, format="gif"
                )
            }
        )
        plt.clf()
