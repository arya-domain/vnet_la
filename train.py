...
class Trainer:
    ...
    def train_epoch(self, model1, model2, optimizer1, optimizer2, iter_num=0):
        ...
        for batch_idx, (normal_batch, cons_batch) in enumerate(self.train_loader):
            ...
            """ model1 training """
            # spatial-consistent
            noise = torch.zeros_like(volume_batch).uniform_(-0.2, 0.2)
            sup_outputs1, _ = model1(volume_batch + noise, tokens_batch)
            ...
            noise = torch.zeros_like(volume_batch).uniform_(-0.2, 0.2)
            unsup_outputs1, _ = model1(volume_batch + noise, tokens_batch)
            ...
            # spatial-consistent
            noise = torch.zeros_like(volume_batch).uniform_(-0.2, 0.2)
            sup_outputs2, _ = model2(volume_batch + noise, tokens_batch)
            ...
            noise = torch.zeros_like(volume_batch).uniform_(-0.2, 0.2)
            unsup_outputs2, _ = model2(cut_mix_input2 + noise, tokens_batch)
