...
def test_all_case(
    ...
    for idx, _ in enumerate(tbar):
        ...
        if np.sum(prediction) == 0:
            ...
            single_metric = calculate_metric_percase(
                np.array(prediction), np.array(label[:])
            )
    ...
    print(
        "|dice={:.4f}|mIoU={:.4f}|ASD={:.4f}|95HD={:.4f}|".format(
            avg_metric[0], avg_metric[1], avg_metric[3], avg_metric[2]
        )
    )
...
def test_single_case(net, image, tokens, stride_xy, stride_z, patch_size, num_classes=1, post_process=False):
...
def test_calculate_metric(ckpt_path, vis=False, post=False):
    net = Vnet(
        n_channels=1, n_classes=2, normalization="batchnorm", has_dropout=True
    ).cuda()
    net.load_state_dict(torch.load(ckpt_path))
    net.eval()
    val_dataset = LAHeartDataset(
        os.path.join(config.code_path, "Dataloader"),
        config.data_path,
        split="eval",
        config=config,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Medical Semi-supervised Semantic Segmentation (valid)"
    )
    parser.add_argument(
        "--env_name",
        default="traCoCo(8-label,spatial_weight(kl)=0.3,hyp=0.1,iters=9000)",
        type=str,
    ...
    parser.add_argument(
        "--post", action="store_true", help="implement post process or not"
    )
    ...
    metric = test_calculate_metric(
        os.path.join(default_path, ckpt), vis=cmd_line.visual, post=cmd_line.post
    )
    print(metric)
